...
for y in years:
  ...
  for s in np.arange(0,len(starts_ukmo)):
    ...
    if curr_start in version_list1:
      ...
      v_year = '2018'
    # year to use (Y-1 for November-December)
    ...
    uk_dates = nc4.num2date(t_time,t_units)# convert time to date using netCDF4 function
    nc_fid.close()
    ...
    for lead in np.arange(0,nleads):
      ...
      if lead == 0: # only open ecmwf and gpcp if lead == 0
        ...
        for l in np.arange(0,nlags_ecmf):
          ...
          if ec_lag_mo in [7,8,9,10,11,12]:
            ec_lag_year = str(y-1)
          else:
            ec_lag_year = str(y)
          ...
          if (s == 0) & (lead == 0) & (l == 0):
            all_ecmf = np.zeros((nleads,len(starts_ukmo),nlags_ecmf,nmembers_ecmf,len(ecmf_lat),len(ecmf_lon)))
            lons_rg_ec, lats_rg_ec = np.meshgrid(ecmf_lon,ecmf_lat)

          if l == 0:
            tmp_ec_lens = np.zeros((nlags_ecmf,nmembers_ecmf,len(ecmf_dates),len(ecmf_lat),len(ecmf_lon)))
      ...
      for l in np.arange(0,nlags_ecmf):
        ...
        ecmf_time_id = np.arange(ecmf_ymdh1_id[0],ecmf_ymd2_id[len(ecmf_ymd2_id)-1]+2)
        # store lagged ensemble
        all_ecmf[lead,s,l,:,:,:] = np.sum(tmp_ec_lens[l,:,ecmf_time_id,:,:],axis=0)/7

  # save ECMWF as netcdf
  nlats = len(ecmf_lat)
  nlons = len(ecmf_lon)
  fname_ncep = dir_out+region+'_ECMWF_ensemble_weekly_hindcasts_lead1-5_DJF_'+str(y)+'.nc'
  ...
  dataset.close()
  print 'saved ecmf file for '+str(y)
