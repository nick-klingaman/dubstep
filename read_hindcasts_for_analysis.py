...
def read_hindcasts(season):
  ...
  for y in years:
    ...
    for mo in np.arange(0,len(starts_mon)):
      if season == 'NDJFM' and starts_mon[mo] in ['08','09','10','11','12']:
        y_ec	= y-1
      else:
        y_ec	= np.copy(y)
    ...
    if season == 'NDJFM':
      for mo in np.arange(0,len(starts_mon_bam)):
        if season == 'NDJFM' and starts_mon_bam[mo] in ['08','09','10','11','12']:
          y_ec	= y-1
        else:
          y_ec	= np.copy(y)
  ...
  if season == 'NDJFM':
    date_mask = np.where((week_mid_dates[:,:,:,1] > 3) & (week_mid_dates[:,:,:,1] < 11))
    date_mask_bam = np.where((week_mid_dates_bam[:,:,:,:,1] > 3) & (week_mid_dates_bam[:,:,:,:,1] < 11))
