...
#     print ukmo_dates.shape			# (239,3)
      ...
      for lead in np.arange(0,5):
        ...
        all_ukmo[lead,s,:,:,:] = np.mean(np.flip(sub_ukmo_rfe,axis=2),axis=1)*4.
...
#           print ncep_dates.shape				# (176)
            ...
            tmp_lens[l,:,:,:,:] = np.copy(np.flip(ncep_rfe,axis=2))
        ...
        for l in np.arange(0,nlags_ncep):
          ...
          all_ncep[lead,s,l,:,:,:] = np.mean(tmp_lens[l,:,ncep_time_id,:,:],axis=0)*4. # units: kg/m2 per day?
        ...
        for d in np.arange(0,len(week_id),4):
          ...
          if (lead == 0) & (s == 0) & (d == 0):
            ...
            chirps_weekly = np.zeros((nleads,len(starts_ukmo),len(ncep_lat),len(ncep_lon)))

          # gpcp4ncep[lead,s,:,:] = gpcp4ncep[lead,s,:,:] + gpcp_rg[::-1,:] # units: mm/week, turn latitude to [20,-40]
          chirps_weekly[lead,s,:,:] = chirps_weekly[lead,s,:,:] + chirps_p[0,:,:] # units: mm/week, turn latitude to [20,-40]
    ...
    latitudes[:]	= np.flip(ukmo_lat,axis=0)
    longitudes[:]	= ukmo_lon
    ...
    # Fill variables with data
    latitudes[:]	= np.flip(ncep_lat,axis=0)
    longitudes[:]	= ncep_lon
    ...
    nlats	= len(ncep_lat)
    nlons	= len(ncep_lon)
    fname_gpcp	= dir_out+region+'_CHIRPS_weekly_'+str(y)+month[m]+'.nc'
    ...
    # Global Attributes (will need modified accordingly)
    dataset.description	= 'CHIRPS weekly mean precipitation at forecast 1.5 degrees, based on weekly UKMO intialisation dates'
    dataset.history	= 'Created ' + tt.ctime(tt.time())
    dataset.source	= 'Subset by L. Guo'
    # Variable Attributes
    latitudes.units	= 'degrees_north'
    ...
    p_mean[:]		= np.swapaxes(chirps_weekly,0,1)/7.			# convert weekly mean of mm d rather than weekly total
    leads[:] = np.arange(1,nleads+1)
    weeks[:] = np.arange(1,len(starts_ukmo)+1)
    dataset.close()
    print ''
