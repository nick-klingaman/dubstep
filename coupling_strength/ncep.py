...
ENS=4 #number of ensemble members
LAG=7 #number of lags
ALL=ENS*LAG
...
#Reading in the precipitaiton (pr), evaporation (et), soil moisture (sm) and 2m temperature (t2) data for NCEP
...
for i in range(len(yrmon)):
    pr[i,:,:,:,:,:]=Dataset('/gws/nopw/j04/ncas_climate_vol1/users/myoung02/datasets/DUBSTEP_data/SAmerica_NCEP_ensemble_weekly_hindcasts_lead1-5_'+yrmon[i]+'.nc', 'r').variables['week_precip'][:]
    et[i,:,:,:,:,:]=Dataset('/gws/nopw/j04/klingaman/amulya/data/surface_s2s_skill/slhf/dubstep_output/NCEP/SA_NCEP_ensemble_ET_weekly_hindcasts_lead1-5_'+yrmon[i]+'.nc', 'r').variables['week_ET'][:]
    sm[i,:,:,:,:,:]=Dataset('/gws/nopw/j04/klingaman/amulya/data/surface_s2s_skill/soilm/dubstep_output/NCEP/SA_NCEP_ensemble_sm20_weekly_hindcasts_lead1-5_'+yrmon[i]+'.nc', 'r').variables['week_sm20'][:]
    t2[i,:,:,:,:,:]=Dataset('/gws/nopw/j04/klingaman/amulya/data/surface_s2s_skill/t2m/dubstep_output/NCEP/SA_NCEP_ensemble_t2m_weekly_hindcasts_lead1-5_'+yrmon[i]+'.nc', 'r').variables['week_t2m'][:]
...
np.savez('../output/ncep', gam=gam, tci=tci, tet=tet, pleg=pleg, tleg=tleg) #saving the coupling strength metrics as a numpy output file for NCEP
